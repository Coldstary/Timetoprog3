    ...
    public partial class Tablecreator : Form
    {
                ...
                using (StreamReader reader = new StreamReader(csvPath, Encoding.UTF8))
                {
                    ...
                    while (!reader.EndOfStream)
                    {
                        ...
                        if (tableType == "Документ" && currentFileData == null)
                        {
                            ...
                            // Вторая строка CSV содержит заголовок в колонке 1 (индекс 3)
                            if (values.Length > 3)
                            {
                        ...
                        {
                            // Добавляем данные строки таблиции
                ...
                if (data != null && data.Count > 0)
                {
                    for (int i = 0; i < data.Count && i < (existingRows - 1); i++)
                    {
                        ...
                        for (int col = 0; col < Math.Min(rowData.Length, table.Columns.Count); col++)
                        {
                            ...
                            // Для пустых значений из CSV оставляем пустую строку
        ...
        private void CreateExcelFileWithNoData(string filePath, int startYear, int endYear)
        {
            ...
            try
            {
                ...
                Excel.Range titleRange = worksheet.get_Range("A1", "A1");
                titleRange.Font.Bold = true;
                titleRange.Font.Size = 14;

                Excel.Range periodRange = worksheet.get_Range("A2", "A2");
                periodRange.Font.Bold = true;
                periodRange.Font.Size = 12;

                Excel.Range messageRange = worksheet.get_Range("A4", "A4");
        ...
        private void CreateChartData(Excel.Worksheet worksheet, Dictionary<string, double> indicators, int startRow)
        {
            ...
            try
            {
                ...
                chartHeaderRange = worksheet.get_Range(
                    worksheet.Cells[startRow, 1],
                    worksheet.Cells[startRow, 2]);
                ...
                chartDataRange = worksheet.get_Range(
                    worksheet.Cells[startRow + 1, 1],
                    worksheet.Cells[startRow + indicators.Count, 2]);
                ...
                column1 = (Excel.Range)worksheet.Columns[1];
                column1.ColumnWidth = 20;

                column2 = (Excel.Range)worksheet.Columns[2];
            ...
            string selectedTeacher, int startYear, int endYear, List<string> teacherFiles)
        {
            try
            {
                ...
                Excel.Range endCell = (Excel.Range)worksheet.Cells[startRow + indicatorCount, 2];
        ...
        private void ApplyExcelFormatting(Excel.Worksheet worksheet, List<string[]> data)
        {
            ...
            try
            {
                ...
                Excel.Range headerRange = worksheet.get_Range(
                    worksheet.Cells[1, 1],
                ...
                Excel.Range dataRange = worksheet.get_Range(
                    worksheet.Cells[1, 1],
                ...
                {
                    Excel.Range rowRange = worksheet.get_Range(
                        worksheet.Cells[row, 1],
                        worksheet.Cells[row, colCount]);
                    ...
                    // Выделение особых строк
                    Excel.Range cellType = (Excel.Range)worksheet.Cells[row, 2]; // Колонка "Номер таблицы"

                    if (cellType.Value != null)
                    {
                        string cellValue = cellType.Value.ToString();

                        if (cellValue == "Документ")
                        {
                            rowRange.Interior.Color = System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.LightBlue);
                            rowRange.Font.Bold = true;
                        }
                        else if (cellValue == "Подпись")
                        {
                            rowRange.Interior.Color = System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.LightGreen);
                            rowRange.Font.Bold = true;
                        }
                        else if (cellValue == "ОШИБКА")
                        {
                            rowRange.Interior.Color = System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.LightCoral);
                            rowRange.Font.Bold = true;
                            rowRange.Font.Color = System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.DarkRed);
                        }
                        else if (cellValue.Contains("---"))
                        {
                            // Строки-разделители
                            rowRange.Interior.Color = System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Gray);
                            rowRange.Font.Bold = true;
                        }
                    }

                    // Освобождаем COM объекты
                    Marshal.ReleaseComObject(cellType);
                ...
                {
                    Excel.Range column = (Excel.Range)worksheet.Columns[col];

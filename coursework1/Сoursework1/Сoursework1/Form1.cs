...
//                chartHeaderRange = worksheet.Range[worksheet.Cells[startRow, 1], worksheet.Cells[startRow, 2]];
...
//                chartDataRange = worksheet.Range[worksheet.Cells[startRow + 1, 1], worksheet.Cells[startRow + indicators.Count, 2]];
...
//            Excel.Shape chartShape = null;
//            Excel.Chart excelChart = null;
...
//                chartShape = shapes.AddChart(Excel.XlChartType.xlColumnClustered,
//                    10, // Left
//                    (startRow + indicatorCount + 3) * 15, // Top
//                    600, // Width
//                    300); // Height

//                excelChart = chartShape.Chart;
...
//                try
//                {
//                    // Настройка категорийной оси (оси X)
//                    excelChart.Axes(Excel.XlAxisType.xlCategory, Excel.XlAxisGroup.xlPrimary).CategoryNames = worksheet.Range[worksheet.Cells[startRow + 1, 1], worksheet.Cells[startRow + indicatorCount, 1]];
//                }
//                catch
//                {
...
//                if (chartShape != null) Marshal.ReleaseComObject(chartShape);
//            }
//        }
...
//                Excel.Range dataRange = worksheet.Range[worksheet.Cells[1, 1], worksheet.Cells[rowCount, colCount]];
...
//                {
//                    Excel.Range rowRange = worksheet.Range[worksheet.Cells[row, 1], worksheet.Cells[row, colCount]];
//                    rowRange.Interior.Color = System.Drawing.ColorTranslator.ToOle(System.Drawing.ColorTranslator.FromHtml("#F0F0F0"));
...
//                Excel.Range filterRange = worksheet.Range[worksheet.Cells[1, 1], worksheet.Cells[1, colCount]];
    ...
    public partial class Tablecreator : Form
    {
        ...
        private void CreateChartBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Сначала откройте файлы через кнопку 'Открыть файлы'", "Информация",
                                  MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                ...
                using (YearSelectionForm yearForm = new YearSelectionForm())
                {
                    ...
                    using (ChartTypeForm chartTypeForm = new ChartTypeForm())
                    {
                        ...
                        foreach (string filePath in _selectedFilePaths)
                        {
                            int fileYear = ExtractYearFromFileName(Path.GetFileName(filePath));
        ...
        private void ButtonExportWord_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (textFromD2.Length > 174)
                {
                    textFromD2 = textFromD2.Substring(0, 174);
                }
        ...
        private void btnOpenFiles_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (result == DialogResult.OK)
                {
                    if (openFileDialog.FileNames.Length > 0)
                    {
                        // Сохраняем выбранные файлы в поле класса
                        _selectedFilePaths = new List<string>(openFileDialog.FileNames);
